{{
  config(
    materialized='table',
    cluster_by=['incident_year', 'crime_against', 'county_name'],
    tags=['intermediate', 'core'],
    meta={
      'owner': 'data-engineering',
      'tier': 'intermediate',
      'pii': false,
      'description': 'Unified, normalized crime incidents from 1997-2024. Single source of truth for all crime fact models. Includes city_name for city-level KPI support.'
    }
  )
}}
...
agency_city_map as (
    select * from {{ ref('agency_city_mapping') }}
),
