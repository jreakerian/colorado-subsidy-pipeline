...
dim_date as (
  select date_key, full_date, date_day
  from {{ ref('dim_date') }}
),

dim_geo as (
  select geo_key, city_name, county_name, zip_code
  from {{ ref('dim_geography') }}
),

dim_offense as (
  select offense_key, offense_category_name, offense_name, crime_against, offense_group
  from {{ ref('dim_offense') }}
),

dim_agency as (
  select agency_key, agency_name, primary_county
  from {{ ref('dim_agency') }}
),
...
-- Dimension surrogate keys
  ...
  left join dim_agency ag
    on c.agency_name = ag.agency_name

  left join dim_offense o
    on c.offense_category_name = o.offense_category_name
  ...
  left join dim_geo g
    on lower(trim(split_part(c.county_name, ',', 1))) = lower(trim(g.county_name))
