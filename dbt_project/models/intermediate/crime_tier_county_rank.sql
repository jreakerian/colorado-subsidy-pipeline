...
select
  ...
  max(case when offense_category_name = 'Burglary/Breaking & Entering' then crime_tier end) as burglary_tier,
  max(case when offense_category_name = 'Larceny/Theft Offenses' then crime_tier end) as larceny_theft_tier,
  max(case when offense_category_name = 'Motor Vehicle Theft' then crime_tier end) as vehicle_theft_tier,
