...
city_rows as (
  select
    ...
    co.cent_lat,
    co.cent_long
  from city_county_zip ccz
  left join county_coordinates co
...
county_only_rows as (
  select
    ...
    lower(trim(co.label))     as county_name,            -- normalise to lowercase
    null                      as zip_code,
    co.cent_lat,
    co.cent_long
  from county_coordinates co
),

combined as (
  select city_name, county_name, zip_code, cent_lat, cent_long from city_rows
  union
  select city_name, county_name, zip_code, cent_lat, cent_long from county_only_rows
),
