...
class Alignment:
    ...
    __slots__ = ("comp", "ha", "has_addition", "has_removal", "hb", "ia", "ib", "index_columns", "map_a2b", "map_b2a", "map_count", "marked_as_identical", "meta", "order_cache", "order_cache_has_reference", "reference", "ta", "tb")
...
class CellInfo:
    ...
    __slots__ = ("category", "category_given_tr", "conflicted", "lvalue", "meta", "pretty_separator", "pretty_value", "pvalue", "raw", "rvalue", "separator", "updated", "value")
...
class CombinedTable:
    ...
    __slots__ = ("body", "core", "dx", "dy", "head", "meta", "t")
...
class CombinedTableBody:
    ...
    __slots__ = ("all", "dx", "dy", "meta", "parent")
...
class CompareFlags:
    ...
    __slots__ = ("acts", "allow_nested_cells", "always_show_header", "always_show_order", "columns_to_ignore", "count_like_a_spreadsheet", "diff_strategy", "ids", "ignore_case", "ignore_epsilon", "ignore_whitespace", "never_show_order", "ordered", "padding_strategy", "parent", "quote_html", "show_meta", "show_unchanged", "show_unchanged_columns", "show_unchanged_meta", "tables", "terminal_format", "unchanged_column_context", "unchanged_context", "use_glyphs", "warnings")
        ...
        if (ids is not None):
            ...
            if (len(common_units) > N):
                ...
                while (_g < _g1):
                    ...
                    while (_g4 < _g5):
                        ...
                        if (key1 not in mem2.h):
                            mem2.h[key1] = 1
                            ct = (ct + 1)
                    columns_eval.append([i, ct])
                def _hx_local_6(a,b):
                    if ((a[1] if len(a) > 1 else None) < (b[1] if len(b) > 1 else None)):
                        return 1
                    if ((a[1] if len(a) > 1 else None) > (b[1] if len(b) > 1 else None)):
                        return -1
                    if ((a[0] if len(a) > 0 else None) > (b[0] if len(b) > 0 else None)):
                        return 1
                    if ((a[0] if len(a) > 0 else None) < (b[0] if len(b) > 0 else None)):
                        return -1
                    return 0
            ...
            while (_g < _g1):
                ...
                while j.hasNext():
                    ...
                    if (val not in used.h):
                        fixed.append(j1)
                        align.link(j1,val)
                        used.set(val,1)
                        if (j1 not in used_reverse.h):
                            reverse_pending_ct = (reverse_pending_ct - 1)
...
class ConflictInfo:
    ...
    __slots__ = ("col", "lvalue", "pvalue", "row", "rvalue")
...
class Coopy:
    ...
    __slots__ = ("cache_txt", "css_output", "csv_eol_preference", "daff_cmd", "delim_preference", "diffs_found", "extern_preference", "fail_if_diff", "flags", "format_preference", "fragment", "io", "mv", "nested_output", "order_preference", "order_set", "output_format", "output_format_set", "status", "strategy")
    ...
    def run(self,args,io = None):
        ...
        try:
            ...
            if git:
                ...
                del args[pos:(pos + _hx_len)]
                offset = 0
                old_display_path = (git_args[0] if len(git_args) > 0 else None)
                new_display_path = (git_args[0] if len(git_args) > 0 else None)
                old_file = (git_args[1] if len(git_args) > 1 else None)
                new_file = (git_args[4] if len(git_args) > 4 else None)
                if (ct == 9):
                    io.writeStdout(git_args[8] if len(git_args) > 8 else None)
                    new_display_path = (git_args[7] if len(git_args) > 7 else None)
    ...
    def coopyhx(self,io):
        args = io.args()
        if ((args[0] if len(args) > 0 else None) == "--keep"):
...
class CrossMatch:
    ...
    __slots__ = ("item_a", "item_b", "spot_a", "spot_b")
...
class Csv:
    _hx_class_name = "Csv"
    __slots__ = ("cursor", "delim", "discovered_eol", "has_structure", "preferred_eol", "row_ended")
    ...
    def renderCell(self,v,d,force_quote = None):
        ...
        if (not need_quote):
            if (len(_hx_str) > 0):
                tmp = None
                if ((("" if ((len(_hx_str) <= 0)) else _hx_str[0])) != " "):
                    index = (len(_hx_str) - 1)
        ...
        if (not need_quote):
            ...
            while (_g < _g1):
                ...
                if ((((ch == "\"") or ((ch == "\r"))) or ((ch == "\n"))) or (ch == "\t")):
                    ...
                    break
                _this = self.delim
                if (ch == ("" if (len(_this) <= 0) else _this[0])):
                    if (len(self.delim) == 1):
                        need_quote = True
                        break
...
class DiffRender:
    ...
    __slots__ = ("open", "pretty_arrows", "quote_html", "section", "td_close", "td_open", "text_to_insert")
    ...
    def examineCell(x,y,view,raw,vcol,vrow,vcorner,cell,offset = None):
        ...
        if (vcol is None):
            vcol = ""
        if (((len(vrow) >= 3) and (((("" if ((len(vrow) <= 0)) else vrow[0])) == "@"))) and ((("" if ((len(vrow) <= 1)) else vrow[1])) != "@")):
        ...
        if (vrow == "!"):
            ...
            if (((vrow.find("->") if ((startIndex is None)) else HxString.indexOfImpl(vrow,"->",startIndex))) >= 0):
                if (not removed_column):
                    tokens = vrow.split("!")
                    full = vrow
                    part = (tokens[1] if len(tokens) > 1 else None)
                    if (part is None):
                        part = full
                    ...
                    if tmp:
                        ...
                        if (len(tokens) >= 2):
                            python_internal_ArrayImpl._set(pretty_tokens, 0, DiffRender.markSpaces((tokens[0] if len(tokens) > 0 else None),(tokens[1] if len(tokens) > 1 else None)))
                            python_internal_ArrayImpl._set(pretty_tokens, 1, DiffRender.markSpaces((tokens[1] if len(tokens) > 1 else None),(tokens[0] if len(tokens) > 0 else None)))
                        if (len(tokens) >= 3):
                            ref = (pretty_tokens[0] if len(pretty_tokens) > 0 else None)
                            python_internal_ArrayImpl._set(pretty_tokens, 0, DiffRender.markSpaces(ref,(tokens[2] if len(tokens) > 2 else None)))
                            python_internal_ArrayImpl._set(pretty_tokens, 2, DiffRender.markSpaces((tokens[2] if len(tokens) > 2 else None),ref))
                        ...
                        if cell.conflicted:
                            cell.pvalue = (tokens[0] if len(tokens) > 0 else None)
        if ((x == 0) and (offset > 0)):
            def _hx_local_6():
                cell.category = "index"
                return cell.category
...
class DiffSummary:
    ...
    __slots__ = ("col_count_final", "col_count_initial", "col_deletes", "col_inserts", "col_renames", "col_reorders", "col_updates", "different", "row_count_final", "row_count_final_with_header", "row_count_initial", "row_count_initial_with_header", "row_deletes", "row_inserts", "row_reorders", "row_updates")
...
class FlatCellBuilder:
    ...
    __slots__ = ("conflict_separator", "flags", "separator", "view")
...
class HighlightPatch:
    ...
    __slots__ = ("actions", "cellInfo", "cmods", "colPermutation", "colPermutationRev", "csv", "currentRow", "destInPatchCol", "finished_columns", "flags", "haveDroppedColumns", "header", "headerMove", "headerPost", "headerPre", "headerRename", "headerRow", "indexes", "lastSourceRow", "meta", "meta_change", "modifier", "mods", "next_meta", "patch", "patchInDestCol", "patchInSourceCol", "patchInSourceRow", "payloadCol", "payloadTop", "preambleRow", "prev_meta", "process_meta", "rcOffset", "rowInfo", "rowPermutation", "rowPermutationRev", "source", "sourceInPatchCol", "sourceView", "view")
    ...
    def setMetaProp(self,target,column_name,prop_name,value):
        ...
        if (prop_name is None):
            return
        if (column_name not in target.h):
            value1 = list()
    ...
    def processMods(self,rmods,fate,_hx_len):
        ...
        target = 0
        if (len(rmods) > 0):
            if ((rmods[0] if len(rmods) > 0 else None).sourcePrevRow == -1):
                last = 0
        _g = 0
        while (_g < len(rmods)):
...
class HighlightPatchUnit:
    ...
    __slots__ = ("add", "code", "destRow", "patchRow", "rem", "sourceNextRow", "sourcePrevRow", "sourceRow", "sourceRowOffset", "update")
...
class Index:
    ...
    __slots__ = ("cols", "hdr", "height", "ignore_case", "ignore_whitespace", "indexed_table", "items", "keys", "top_freq", "v")
...
class IndexItem:
    ...
    def value(self):
        return (self.lst[0] if len(self.lst) > 0 else None)
...
class IndexPair:
    ...
    __slots__ = ("flags", "hdr", "ia", "ib", "quality")
...
class JsonTable:
    ...
    __slots__ = ("columns", "data", "h", "idx2col", "name", "rows", "w")
...
class Merger:
    ...
    __slots__ = ("column_mix_local", "column_mix_remote", "column_order", "column_units", "conflict_infos", "conflicts", "flags", "local", "order", "parent", "remote", "row_mix_local", "row_mix_remote", "units")
...
class Ndjson:
    ...
    __slots__ = ("columns", "header_row", "tab", "view")
...
class RowChange:
    ...
    __slots__ = ("action", "cond", "conflicted", "conflicting_parent_val", "conflicting_val", "is_key", "val")
...
class SimpleMeta:
    ...
    __slots__ = ("has_properties", "keys", "may_be_nested", "metadata", "name2col", "name2row", "row_active", "row_change_cache", "t")
    ...
    def addMetaData(self,column,property,val):
        ...
        if (column not in self.metadata.h):
            this1 = self.metadata
    ...
    def asTable(self):
        ...
        while (_g < _g1):
            ...
            if (name not in self.metadata.h):
                continue
...
class SqlColumn:
    ...
    __slots__ = ("name", "primary", "type_family", "type_value")
...
class SqlCompare:
    ...
    __slots__ = ("align", "alt", "alt_peered", "at0", "at1", "at2", "db", "diff_ct", "flags", "local", "needed", "peered", "remote")
        ...
        while (_g < _g1):
            ...
            if (key in present1.h):
                if (key in present2.h):
                    ...
                    if (key not in present_primary.h):
...
class SqlTable:
    ...
    __slots__ = ("cache", "columnNames", "columns", "db", "h", "helper", "id2rid", "name", "quotedTableName")
    ...
    def splitSchema(self,db,name,sql):
        ...
        while (_g < _g1):
            ...
            if (ch == ","):
                ...
                tmp = (nesting == 1)
            if brk:
                if ((("" if ((len(token) <= 0)) else token[0])) == " "):
...
class Std:
    ...
    def parseInt(x):
        ...
        if isHexadecimal:
            while (cur < _hx_len):
                if (x[cur] not in "0123456789abcdefABCDEF"):
                    break
                cur = (cur + 1)
        else:
            while (cur < _hx_len):
                if (x[cur] not in "0123456789"):
                    break
                cur = (cur + 1)
...
class TableComparisonState:
    ...
    __slots__ = ("a", "a_meta", "alignment", "b", "b_meta", "child_order", "children", "compare_flags", "completed", "has_same_columns", "has_same_columns_known", "is_equal", "is_equal_known", "p", "p_meta", "run_to_completion")
...
class TableDiff:
    ...
    __slots__ = ("a", "act", "active_column", "active_row", "align", "allow_column", "allow_delete", "allow_insert", "allow_update", "b", "builder", "col_deletes", "col_inserts", "col_map", "col_moves", "col_renames", "col_reorders", "col_updates", "column_units", "column_units_updated", "conflict_sep", "diff_found", "flags", "has_parent", "have_addition", "have_schema", "is_index_a", "is_index_b", "is_index_p", "nested", "nesting_present", "order", "p", "preserve_columns", "publish", "ra_header", "rb_header", "row_deletes", "row_inserts", "row_map", "row_moves", "row_reorders", "row_units", "row_updates", "rp_header", "schema", "schema_diff_found", "sep", "show_rc_numbers", "top_line_done", "v")
    ...
    def checkNesting(self,v,have_ll,ll,have_rr,rr,have_pp,pp,x,y):
        ...
        if (self.align.comp is not None):
            if (self.align.comp.children is None):
                ...
                compare = (key not in self.align.comp.children.h)
        if compare:
    ...
    def scanRow(self,unit,output,at,i,out):
        ...
        while (_g < _g1):
            ...
            if self.nested:
                ...
                ll = (ndiff[0] if len(ndiff) > 0 else None)
                rr = (ndiff[1] if len(ndiff) > 1 else None)
                pp = (ndiff[2] if len(ndiff) > 2 else None)
            if have_pp:
                if (not have_rr):
                    dd = pp
                ...
                if (not have_rr) or self.isEqual(self.v,ll,rr):
                    dd = ll
                else:
                    dd = ll
                    dd_to = rr
            ...
            if (have_dd_to and (((dd is not None) and self.allow_update) or self.allow_column)):
                ...
                if (j not in self.column_units_updated.h):
...
class TableStream:
    ...
    __slots__ = ("at", "columns", "h", "row", "src", "t")
...
class Tables:
    ...
    __slots__ = ("alignment", "table_order", "tables", "template")
    ...
    def one(self):
        return self.tables.h.get((self.table_order[0] if len(self.table_order) > 0 else None),None)
...
class TerminalDiffRender:
    ...
    __slots__ = ("align_columns", "codes", "csv", "delim", "diff", "flags", "t", "use_glyphs", "v", "wide_columns")
...
class Type:
    ...
    def typeof(v):
        ...
        if (isinstance(v,_hx_AnonObject) or python_lib_Inspect.isclass(v)):
            return ValueType.TObject
        if isinstance(v,Enum):
            return ValueType.TEnum(v.__class__)
        if (isinstance(v,type) or hasattr(v,"_hx_class")):
...
class Viterbi:
    ...
    __slots__ = ("K", "T", "best_cost", "cost", "index", "mode", "path", "path_valid", "src")
...
class haxe_Exception(Exception):
    ...
    __slots__ = ("_hx___nativeException", "_hx___nativeStack", "_hx___previousException", "_hx___skipStack")
    ...
    def caught(value):
        if Std.isOfType(value,haxe_Exception):
            return value
        if Std.isOfType(value,BaseException):
...
class haxe_format_JsonPrinter:
    ...
    __slots__ = ("buf", "indent", "nind", "pretty", "replacer")
...
class sys_io_File:
    ...
    @staticmethod
    def getContent(path):
        f = python_lib_Builtins.open(path,-1,"utf-8",None,"")
        content = f.read(-1)
        f.close()
        return content
